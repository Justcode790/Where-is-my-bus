...
const RouteManagement = () => {
  ...
  useEffect(() => {
    const fetchRoutes = async () => {
      try {
        const res = await API.get("http://localhost:5000/pi/official/routes");
  ...
  const handleAddRoute = async (e) => {
    ...
    const newRoute = {
      ...
      stops: e.target.stops.value.split(",").map(s => s.trim()), // convert to array
      distance: e.target.distance.value,
      estimatedTime: "N/A" // you can add input if needed
    };

    try {
      const res = await axios.post("http://localhost:5000/pi/official/buses/routes", newRoute);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <tbody>
              {routes.map(route => (
                <tr key={route._id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-2">{route.stops.join(", ")}</td>
      ...
      {/* <div className="bg-white p-6 rounded-lg shadow">
        ...
        <form onSubmit={handleAddRoute}>
          ...
          <div className="mb-4">
            <label htmlFor="stops" className="block text-sm font-medium text-gray-700 mb-1">Stops (comma separated)</label>
            <input type="text" id="stops" name="stops" required className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g., Majestic, MG Road, KR Puram, ITPL" />
          </div>
          <div className="mb-4">
            <label htmlFor="distance" className="block text-sm font-medium text-gray-700 mb-1">Distance (km)</label>
            <input type="text" id="distance" name="distance" required className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g., 22" />
          </div>
